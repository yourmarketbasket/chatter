...
class _SearchPageState extends State<SearchPage> {
  ...
  void _performSearch() {
    ...
    // Directly use the posts from the controller, assuming they are List<Map<String, dynamic>>
    final allPosts = _dataController.posts;

    final filteredPosts = allPosts.where((postMap) {
      final content = postMap['content'] as String? ?? '';
      final username = postMap['username'] as String? ?? '';
  ...
  Widget _buildPostItem(Map<String, dynamic> postMap) {
    ...
    // Extract data from postMap, providing defaults where necessary
    final String username = postMap['username'] as String? ?? 'Unknown User';
    final String content = postMap['content'] as String? ?? '';
    final String? useravatar = postMap['useravatar'] as String?;
    final String avatarInitial = username.isNotEmpty ? username[0].toUpperCase() : '?';
    final DateTime timestamp = postMap['createdAt'] is String
        ? DateTime.parse(postMap['createdAt'])
        : DateTime.now();
    final List<dynamic> attachments = postMap['attachments'] as List<dynamic>? ?? [];
